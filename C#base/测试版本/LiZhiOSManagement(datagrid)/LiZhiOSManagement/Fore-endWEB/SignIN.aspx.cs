...
namespace LiZhiOSManagement.Fore_endWEB
{
    public partial class SignIN : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Response.Write("<script language=javascript>alert('点击确定登录页面，请登录后操作！');window.location='/Logins.aspx'</script>");
            }
            else
            {
    ...
    24:DateTime.Now.ToString("HH:mm:ss")*/
                ...
                getIP = GetLocalIp();//获取本地客户机IP
                if (getIP == userIP)//IP验证合格后进行签到操作
                {
                ...
                //else
                //{
                //    Response.Write("<script language=javascript>alert('请使用指定IP进行签到！\n 若IP变更，请联系管理员！');window.location='/Indexs.aspx'</script>");
                //}
                    string names = Session["UserFName"].ToString();
                    ...
                    foreach (DataRow dr in showList.Rows)
                    {
                        ...
                        if (pd == toDayTime)
                        {
                            ...
                            if (dr["name"].ToString() == names)
                                tTr.Append(" <td><font style='font-weight:bold'>" + dr["name"] + "</font></td>");
                            else
