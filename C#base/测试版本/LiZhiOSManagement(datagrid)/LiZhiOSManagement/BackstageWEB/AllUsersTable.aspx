<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AllUsersTable.aspx.cs" Inherits="LiZhiOSManagement.WEB.AllUsersTable" %>
...
<link href="JQueryEasyUI/themes/default/easyui.css" rel="stylesheet" />
    ...
    <script>
        ...
        function InitList() {
            $("#tt").datagrid({
                url: 'Ajax/TableByUsersDatagrid.ashx?belongkind=All&bysel=ID&byserch=*',
                title: '数据列表',
                //width: 1190,
                //height: 460,
                fitColumns: true,
                ...
                onCheck: function (rowIndex, rowData) {       //选中n行
                    ...
                    if (selrow != null) {
                        ...
                        $.get('Ajax/SelectTimeForUsersEdit.ashx?id=' + selrow[0].ID, function (data) {
                            $('.ctime').val(data);
                        });
                        ...
                        $('#ebtnadd').bind('click', function () {
                            //alert("进入");
                            $.post('Ajax/SaveEditUsers.ashx', { id: $('.cid').val(), isdelete: $('.isdel').val(), createtime: $('.ctime').val(), username: $('.cname').val(), userpassword: $('.cpwd').val(), usersex: $('.csex').val(), userqq: $('.cqq').val(), userphone: $('.cphone').val(), userstuid: $('.cstuid').val(), userfname: $('.cfname').val(), userip: $('.cip').val(), belong: $(".cgroup").val() }, function (data) {
                                if (data > 0) {
                                    alert("编辑成功!");
                    ...
                    if ($('#tt').datagrid('getChecked').length <= 0) {
                        ...
                        $('#dabtnadd').bind('click', function () {
                            if ($('#tt').datagrid('getChecked').length > 0) {
                                ...
                                $.get('Ajax/DeleteUser.ashx', { shuzu: sz }, function (data) {
                ...
                onUncheck: function (rowIndex, rowData) {
                    ...
                    if ($('#tt').datagrid('getChecked').length > 0) {
                        ...
                        $.get('Ajax/SelectTimeForUsersEdit.ashx?id=' + $('#tt').datagrid('getChecked')[0].ID, function (data) {
                            $('.ctime').val(data);
                        });
                        ...
                        $('#dabtnadd').bind('click', function () {
                            if ($('#tt').datagrid('getChecked').length > 0) {
                                ...
                                $.get('Ajax/DeleteUser.ashx', { shuzu: sz }, function (data) {
                                    if (data == 110) {
                                        alert("删除成功");
                                        window.location.href = "BackendLogin.aspx";
                                    }
                                    else if (data == 1) {
                                        alert("删除成功");
                                        window.location.reload();
                ...
                , '-', {
                    ...
                    handler: function () {                                                   //设置用户为管理员
                        ...
                        if ($('#tt').datagrid('getChecked').length > 0) {
                            ...
                            $.get('Ajax/AddManagers.ashx', { shuzu: sz }, function (data) {
                            ...
                            //var i=0;
                            //for ( i; i < $('#tt').datagrid('getChecked').length; i++)
                            //{
                            //    $.get('Ajax/SelectTimeForUsersEdit.ashx?id=' + $('#tt').datagrid('getChecked')[i].ID, function (data) {
                            //        $('.ctime').val(data);
                            //    });
                            //    $.post('AddManagers.ashx', { isdelete: $('#tt').datagrid('getChecked')[i].IsDelete, createtime: $('.ctime').val(), name: $('#tt').datagrid('getChecked')[i].UserName, pwd: $('#tt').datagrid('getChecked')[i].UserPassWord, sex: $('#tt').datagrid('getChecked')[i].UserSex, qq: $('#tt').datagrid('getChecked')[i].UserQQ, phone: $('#tt').datagrid('getChecked')[i].UserPhone, stuid: $('#tt').datagrid('getChecked')[i].UserStuID, fname: $('#tt').datagrid('getChecked')[i].UserFname, ip: $('#tt').datagrid('getChecked')[i].UserIP, group: $('#tt').datagrid('getChecked')[i].Belong }, function (data) {
                            //        if (data < 1) {
                            //            alert($('#tt').datagrid('getChecked')[i].UserName+"设置失败");
                            //        }
                            //    })
                            //}
                            //if ($('#tt').datagrid('getChecked').length == i)
                            //{
                            //    alert("设置成功");
                            //}
                ...
                , '-', {
                    ...
                    handler: function () {
                        ...
                        if ($('#searchin').val().length > 0) {
                            alert($('#searchsel').val());
                            alert($('#searchin').val());
                            $("#tt").datagrid({ url: "Ajax/TableByUsersDatagrid.ashx?belongkind=All&bysel=" + $('#searchsel').val() + "&byserch=" + $('#searchin').val() });
                            $("#tt").datagrid('reload');
                        }
                        else {
                            alert("搜索框不能为空");
                    ...
                    handler: function () {
                        ...
                        if ($('#tt').datagrid('getChecked').length > 0) {
                            ...
                            window.location.href = "Ajax/UsersExcelOutPut.ashx?id=" + list;
                        }
                        else {
                    ...
                    iconCls: 'icon-pie',
                    handler: function () {
                        window.location.href = "GenderAnalysis.aspx";
                    }
                }]
            });
        }
        ...
        $(function () {
            ...
            $('#abtnadd').bind('click', function () {
                ...
                if (zong.length != 0 && ($("select").find("option:selected").val().length != 0) && ($('.name').val().length != 0) && ($('.pwd').val().length != 0) && ($('.sex').val().length != 0) && ($('.qq').val().length != 0) && ($('.phone').val().length != 0) && ($('.stuid').val().length != 0) && ($('.fname').val().length != 0) && ($('.ip').val().length != 0) && ($('.isdelete').val().length != 0)) {
                    $.post('Ajax/AddUsersByManager.ashx', { isdelete: $('.isdelete').val(), createtime: zong, name: $('.name').val(), pwd: $('.pwd').val(), sex: $('.sex').val(), qq: $('.qq').val(), phone: $('.phone').val(), stuid: $('.stuid').val(), fname: $('.fname').val(), ip: $('.ip').val(), group: $(".group").val() }, function (data) {
                        if (data > 0) {
                            alert("添加成功!");
...
<body class="easyui-layout">
    ...
    <table style="margin-left:auto;margin-right:auto;vertical-align:middle;margin-top:20px;">
        ...
        <tr><td>组别</td><td>
            ...
            <%
                string quan = Session["quan"].ToString() == null ? null :Session["quan"].ToString();              //登陆人的权限
                if(quan=="sm")
                {
                    %>
