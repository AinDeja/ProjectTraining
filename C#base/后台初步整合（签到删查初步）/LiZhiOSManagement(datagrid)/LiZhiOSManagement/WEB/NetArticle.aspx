<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NetArticle.aspx.cs" Inherits="LiZhiOSManagement.WEB.NetArticle" %>
...
<link href="JQueryEasyUI/themes/default/easyui.css" rel="stylesheet" />
    ...
    <script>
        ...
        function InitList() {
            $("#tt").datagrid({
                url: 'Ajax/TableByDatagrid.ashx?belongkind=.Net&bysel=ID&byserch=*',
                title: '数据列表',
                fitColumns: true,
                idField: 'ID',
                loadMsg: '正在加载成员的信息...',
                ...
                toolbar: [{
                    ...
                    handler: function () {
                        ...
                        if ($('#searchin').val().length > 0) {
                            alert($('#searchsel').val());
                            alert($('#searchin').val());
                            $("#tt").datagrid({ url: "Ajax/TableByDatagrid.ashx?belongkind=.NET&bysel=" + $('#searchsel').val() + "&byserch=" + $('#searchin').val() });
                            $("#tt").datagrid('reload');
                        }
                        else {
                            alert("搜索框不能为空");
